...
export const standardizeSpasmEventBodyV2 = (
  event: SpasmEventBodyV2,
  version: string = "2.0.0"
): SpasmEventV2 | null => {
  if (!isObjectWithValues(event)) return null
  if (!("type" in event) || !event.type) return null
  if (event.type !== "SpasmEventBodyV2") return null
    ...
    siblings: [
      {
        ...
        protocol: {
          ...
          version: event.protocol?.version
            ? event.protocol?.version as SpasmVersion
            : "2.0.0"
        },
  ...
  if (
    "parent" in event && event.parent &&
    isObjectWithValues(event.parent)
  ) {
    if (
      "ids" in event.parent && event.parent.ids &&
      isArrayWithValues(event.parent.ids)
    ) {
      spasmEventV2.parent = { ids: event.parent.ids }
      if (
        "marker" in event.parent && event.parent.marker &&
  ...
  if (
    event.license && typeof(event.license) === "string"
  ...
  if (
    spasmEventV2 && isObjectWithValues(spasmEventV2)
  ) { return spasmEventV2 } else { return null }
}

export const standardizeSpasmEventBodySignedClosedV2 = (
  event: SpasmEventBodySignedClosedV2,
  version = "2.0.0"
): SpasmEventV2 | null => {
  if (!isObjectWithValues(event)) return null
  ...
  const spasmEventBodyV2: SpasmEventBodyV2 | null =
    JSON.parse(event.signedString)
  if (!spasmEventBodyV2) return null

  const spasmEventBodyV2ConvertedToSpasmV2: SpasmEventV2 | null =
    standardizeSpasmEventBodyV2(spasmEventBodyV2)
  if (!spasmEventBodyV2ConvertedToSpasmV2) return null

  const eventSignedConvertedToSpasmV2: SpasmEventV2 = {
  ...
  if (spasmEventV2) {
    ...
    spasmEventV2.siblings[0] ??=
      {type:"SiblingSpasmSignedV2"} as SiblingSpasmSignedV2;
    spasmEventV2.siblings[0] =
      spasmEventV2.siblings[0] as SiblingSpasmSignedV2;
...
export const standardizeDmpEventSignedClosedV2 = (
  ...
  const dmpEventConvertedToSpasmV2: SpasmEventV2 | null =
    standardizeDmpEventV2(dmpEvent)
...
export const getTagMappedDetails = (
  tag: string[], 
  tags: string[][]
): string[][] | null => {
  if (!tag) return null
  if (!isArrayOfStrings(tag)) return null
  if (!tags) return null
  if (!isArrayWithValues(tags)) return null

  if (!tag[0] || typeof(tag[0]) !== "string") return null
  if (!tag[1] || typeof(tag[0]) !== "string") return null
  ...
  if (isArrayWithValues(mappedDetails)) {
    return mappedDetails
  }
  return null
}

export const isMappedTag = (
  tag: string[], 
  tags: string[][]
): boolean => {
...
export const getOriginalFromValueAndMethod = (
  value: string | null,
  method?: string | null,
...
export const getOriginalValueFromTagMappedDetails = (
  tag: string[]
): string | null => {
  if (
    !tag || !isArrayOfStrings(tag) ||
    tag[0] !== "tags_mapping" ||
    !tag[1] || typeof(tag[1]) !== "string"
  ) { return null }

  let value: string | undefined | null
  let method: string | undefined | null
  let original: string | undefined | null
...
export const standardizeNostrEventV2 = (
  ...
  // Mapped
  let rootEventsMapped: SpasmEventBodyRootV2[] = []
  let parentEventsMapped: SpasmEventBodyParentV2[] = []
  let referencedEventsMapped: SpasmEventBodyReferenceV2[] = []
  let mentionedAuthorsMapped: SpasmEventMentionV2[] = []
  let extraAuthorsMapped: SpasmEventMentionV2[] = []

  // Unmapped
  let rootEvents: SpasmEventBodyRootV2[] = []
  let parentEvents: SpasmEventBodyParentV2[] = []
  let referencedEvents: SpasmEventBodyReferenceV2[] = []
  let mentionedAuthors: SpasmEventMentionV2[] = []
  let extraAuthors: SpasmEventMentionV2[] = []
  ...
  if (event.tags && Array.isArray(event.tags)) {
    uniqueTags = removeDuplicateNostrTags(event.tags)
  }
  if (
    uniqueTags && Array.isArray(uniqueTags) &&
    isArrayWithValues(uniqueTags)
  ) {
    event.tags.forEach(function (tag) {
      ...
      if (
        ...
        // objects.
        const arrayOfTagDetails: string[][] | null =
          getTagMappedDetails(tag, event.tags)

        if (
          arrayOfTagDetails &&
          isArrayWithValues(arrayOfTagDetails)
        ) {
          arrayOfTagDetails.forEach((
            tagDetails: string[] | null
          ) => {

            if (
              tagDetails && isArrayOfStrings(tagDetails) &&
              tagDetails[1] === "spasm_id_1" &&
              tagDetails[2] === tag[0] && tagDetails[3] === tag[1] &&
              tagDetails[4] === "se2"
            ) {
              const originalValue =
                getOriginalValueFromTagMappedDetails(tagDetails)
              ...
              if (
                ...
                if (formatName && typeof(formatName) === "string") {
                  eventId.format ??= {
                    name: formatName as SpasmEventIdFormatNameV2
                  };
                  eventId.format.name =
                    formatName as SpasmEventIdFormatNameV2
                }
                  ...
                  if (
                    ...
                    const pubkeyValue =
                      getOriginalFromValueAndMethod(
                        pubkey, pubkeyMethod, pubkeyOriginal
                    )
                    if (
                      pubkeyValue && typeof(pubkeyValue) === "string"
                    ) { eventId.pubkey = pubkeyValue }
                  }
                }

                const hosts: SpasmEventHostV2[] = []
                if (
                  tagDetails[13] &&
                  typeof(tagDetails[13] === "string")
                ) {
                  const hostsStrings: string[] =
                    tagDetails[13].split(',')
      ...
      if (
        ...
        // <event-id>
        const referencedEvent: SpasmEventBodyReferenceV2 = {
          ids: [
            {
              value: toBeHex(tag[1]),
              // Create a new format field only if a
          ...
          arrayOfTagDetails.forEach((
            ...
            if (
              ...
              if (
                ...
                // <pubkey>
                const address: SpasmEventAddressV2 = {
                  value: originalValue,
                  // Create a new format field only if a
                  // format can be determined from a string.
                  ...(
                ...
                if (formatName && typeof(formatName) === "string") {
                  address.format ??= {
                    name: formatName as SpasmEventAddressFormatNameV2
                  };
                  address.format.name =
                    formatName as SpasmEventAddressFormatNameV2
                }
  ...
  if (
    parentEvents && parentEvents[0] && hasValue(parentEvents)
  ) {
    ...
    if (
      rootEvents && rootEvents[0] && hasValue(rootEvents)
    ) {
...
export const standardizeNostrSpasmEventV2 = (
  ...
  let spasmTimestamp: number | null = null
  ...
  if (event.tags &&
    ...
    event.tags.forEach(function (tag) {
      ...
      if (Array.isArray(tag) && tag[0] === "spasm_timestamp") {
        if (Number(tag[1])) { spasmTimestamp = Number(tag[1]) }
      }
      ...
      if (
        Array.isArray(tag) && tag[0] === "spasm_tips" &&
        tag[1] && typeof(tag[1]) === "string"
      ) {
        const tip: SpasmEventBodyTipsV2 = { address: tag[1] }
        if (tag[2] && typeof(tag[2]) === "string") {
          tip.text = tag[2]
        }
        if (tag[3] && typeof(tag[3]) === "string") {
      ...
      if (Array.isArray(tag) && tag[0] === "spasm_category") {
        if (tag[1] && typeof(tag[1]) === "string") {
          const category: SpasmEventCategoryV2 = {
            name: tag[1]
          }
          if (tag[2] && typeof(tag[2]) === "string") {
            category.sub = {
  ...
  if (spasmTimestamp) { spasmEventV2.timestamp = spasmTimestamp }
...
export const standardizeNostrSpasmEventSignedOpenedV2 = (
  ...
  const spasmEventV2: SpasmEventV2 | null =
    standardizeNostrSpasmEventV2(event)
  if (!spasmEventV2) return null
