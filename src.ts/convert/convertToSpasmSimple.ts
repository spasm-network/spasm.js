...
import {
  ...
  hasValue,
  isObjectWithValues,
  mergeConvertToSpasmSimpleConfigs
} from "../utils/utils.js"

import { convertToSpasm } from "./convertToSpasm.js"

export const convertManyToSpasmSimple = (
  unknownEvents: (UnknownEventV2 | SpasmSimpleV2 )[],
  ...
  try {
    ...
    const convertedEvents: SpasmSimpleV2[] = []
    unknownEvents.forEach(event => {
      const convertedEvent = convertToSpasmSimple(
        event, customConfig
      )
...
export const convertToSpasmSimple = (
  unknownEvent: UnknownEventV2 | SpasmSimpleV2,
  customConfig?: CustomConvertToSpasmSimpleConfig
): SpasmSimpleV2 | null => {
  try {
    const defaultConfig = new ConvertToSpasmSimpleConfig()
    const config: ConvertToSpasmSimpleConfig =
      mergeConvertToSpasmSimpleConfigs(
    ...
    if (config.version && config.version === "2.0.0") {
      const SpasmEventSimpleV2 =
        convertSpasmEventV2ToSpasmSimpleV2(
          spasmEventV2, config
      )
...
const convertSpasmEventV2ToSpasmSimpleV2 = (
  spasmEvent: SpasmEventV2,
  config?: CustomConvertToSpasmSimpleConfig
) => {
  ...
  if (
    spasmEvent.source && "name" in spasmEvent.source &&
  ...
  if (isObjectWithValues(simple)) return simple
