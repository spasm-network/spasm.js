...
export const convertToNostr = (unknownEvent, nostrSpasmVersion = "2.0.0") => {
    ...
    if (spasmEventV2.type === "SpasmEventV2" &&
        ...
        const NostrSpasmEventV2 = convertSpasmEventV2ToNostrSpasmV2(spasmEventV2);
        return NostrSpasmEventV2;
    }
    return null;
};
export const convertSpasmEventV2ToNostrSpasmV2 = (spasmEventV2) => {
    if (!("timestamp" in spasmEventV2) || !spasmEventV2.timestamp ||
        !isStringOrNumber(spasmEventV2.timestamp) ||
        !Number(spasmEventV2.timestamp)) {
        return null;
    }
    const timestampShort = toBeShortTimestamp(Number(spasmEventV2.timestamp));
    if (!timestampShort)
        return null;
    ...
    if ("references" in spasmEventV2 && spasmEventV2.references) {
        spasmEventV2.references.forEach((ref, eventIndex) => {
            if (ref.ids && isArrayWithValues(ref.ids) &&
                String(eventIndex)) {
...
const assembleAndPushIdTagMapped = (unsortedIds, tags, flag = "parent.ids", eventIndex = "0") => {
    if (!unsortedIds || !isArrayWithValues(unsortedIds)) {
        return;
    }
...
const assembleAndPushAddressesTagMapped = (addresses, tags, flag = "mentions.addresses", eventIndex = "0") => {
    if (!addresses || !isArrayWithValues(addresses)) {
        return;
    }
    if (!tags || !Array.isArray(tags)) {
        return;
