...
export const convertRssFeedToSpasm = async (rssFeedUnknown, source, customConfig) => {
    if (!rssFeedUnknown)
        return [];
    let rssFeed = {};
    ...
    if (rssItems && isArrayWithValues(rssItems)) {
        const spasmEvents = convertRssItemsToSpasm(rssItems, source, customConfig);
...
export const parseRssFeedString = async (xmlString) => {
    try {
...
export const convertRssItemsToSpasm = (items, source, customConfig) => {
    try {
        ...
        items.forEach(item => {
            ...
            if ("spasmEnvelope" in item && item.spasmEnvelope &&
                ...
                const spasmEvent = convertToSpasm(spasmEnvelope, customConfig);
                ...
                const spasmEvent = convertToSpasm(post, customConfig);
                if (spasmEvent && isObjectWithValues(spasmEvent)) {
                    spasmEvents.push(spasmEvent);
                }
            }
...
const extractItemsFromParsedFeedJson = (feedJson) => {
    if (!feedJson || !isObjectWithValues(feedJson))
        return [];
    // RSS
